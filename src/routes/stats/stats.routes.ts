...
app.post('/user/gamesession', async (req: any, res: any) => {
    ...
    if (isNaN(userId) || isNaN(score)) {
        return res.status(400).send('userId, score, and timePlayed must be numeric');
    }
...
app.get('/user/statistics', async (req: any, res: any) => {
...
app.delete('/user/gamesessions', async (req: any, res: any) => {
    const { userId } = req.query;
...
app.get('/top-scores', async (req: any, res: any) => {
